...
nlcor(X1, data$y, plt = T)
nlcor(X2, data$y, plt = T)
nlcor(X3, data$y, plt = T)
nlcor(X4, data$y, plt = T)
nlcor(X5, data$y, plt = T)
nlcor(X6, data$y, plt = T)
nlcor(X7, data$y, plt = T)
nlcor(X8, data$y, plt = T)
nlcor(X9, data$y, plt = T)
nlcor(X23, data$y, plt = T)

nlcor(X19, data$y, plt = T)
...
gbmFit1 <- train( y ~ X1+ X2+ X4 + X6 + X7 + X50, data = training, 
                  method = "gbm", 
...
write.csv(example_pred,file='gam.csv',row.names=FALSE)
